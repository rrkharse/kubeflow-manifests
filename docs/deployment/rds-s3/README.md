...
5. Configure the pipeline manifest parameters.
    - [RDS] Configure `awsconfigs/apps/pipeline/rds/params.env` file with the `RDS endpoint URL`.
        ...
        ```
    - [S3] Configure `awsconfigs/apps/pipeline/s3/params.env` file with the `S3 bucket name`, and `S3 bucket region`.
